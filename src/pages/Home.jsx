...
export default function Home() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (index >= images.length) {
        setIndex((prev) =>
          prev === images.length ? images.length + 1 : images.length
        );
      } else {
  ...
  const currentImage =
    index < images.length
      ? images[index]
      : altImages[(index - images.length) % 2];
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-br from-gray-900 via-emerald-800 to-emerald-200 p-8">
      ...
      <div className="mt-8 grid grid-cols-3 sm:grid-cols-3 gap-6">
        ...
        <a
          href="https://open.spotify.com/user/cqjiv06y90p950gwgabnog682?si=4dbe599d95f84eae"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
